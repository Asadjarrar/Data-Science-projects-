...
class Car:
    ...
    def accelerate(self, time_interval):
        # self.keeping_safe_distance(simulation, time_interval)
        if self.speed < 15:
...
class Simulation:
    ...
    def car_arrival_lane_A(self):
        ...
        for car in self.lane_A:
            if car.position < speed * 2:
                time.sleep(random.expovariate(1 / INTERARRIVAL_HIGHWAY) + 0.5)
    ...
    def find_nearest_car(self, cars):
        nearest_car = None
        min_distance = float('inf')
        for car in cars:
            if car != self:
                distance = abs(car.position - self.position)
    ...
    def changed_destination(self, car):
        ...
        if cars_lane_slower and car.position < 680:
    ...
    def early_merge(self, car, time_interval):
        if car.on_ramp:
            ...
            merge_position = car.position >= 250 and car.position <= 430  # Allows early merging if close to end of ramp
    ...
    def late_merge(self, car, time_interval):
        ...
        if car.on_ramp:
            ...
            target_lane = self.lane_B
            late_position = car.position > 430 and car.position < 460
            sharp_position = car.position >= 460 or car.speed < 3

            if late_position and not sharp_position:
                safe_to_merge_above = True
                safe_to_merge_behind = True
                car_behind = None
                car_above = None
                for lane_car in target_lane:
                    if lane_car.position < car.position:
                        car_behind = lane_car
                        # Ensure sufficient gap exists
                        safe_distance = (car_behind.speed + abs(
                            car.speed - car_behind.speed)) * 2 + car.car_length * 2  # 2 sec rule
                        if abs(lane_car.position - car.position) < safe_distance:  # Safe merging distance
                            safe_to_merge_behind = False
                            break
                        ...
                        car_above = lane_car
                        # Ensure sufficient gap exists
                        safe_distance = (car.speed + abs(
                            car.speed - car_above.speed)) * 2 + car.car_length * 2  # 2 sec rule
                        if abs(lane_car.position - car.position) < safe_distance:  # Safe merging distance
                            safe_to_merge_above = False
                            break
                ...
                if (safe_to_merge_above and safe_to_merge_behind and is_first_to_merge) or car.merge:
                    car.merge = True
                    if car.y >= 85:
                        car.merged_ramp = True
                        car.on_ramp = False
                        ...
                        target_lane.append(car)
                        car.merge = False
                        print(f"Car late merged at position {car.position}")
                ...
                if (safe_to_merge and is_first_to_merge) or car.merge:
                    ...
                    if car.y >= 85:
                        ...
                        # if not sa:
                        #     car_behind.speed = max(car_behind.speed - 2*DECELERATION_RATE, 0)
                        # if not safe_to_merge_above_sharp:
                        #   car.speed = max(car.speed - 2*DECELERATION_RATE, 0)
    ...
    def update(self):
        ...
        for car in list(self.highway):  # Using list to avoid modification during iteration
            # if car.speed < 5 and car.keeping_safe_distance(self, self.interval):
            #   print("Why are you so slow?")
            if car.position < 790:
            ...
            #     car.accelerate(self.interval)
            # if car in list(self.upstream
            car.move(self, self.interval)
            # car.keeping_safe_distance(self,self.interval)
            if car.position < 790:
                self.changed_destination(car)
            # if car in list(self.ramp):
            #   car.move_ramp(self.interval)
            if car.on_ramp:
                # car.accelerate(self.interval)
                car.move_ramp(self, self.interval)
                self.early_merge(car, self.interval)
                self.late_merge(car, self.interval)
    ...
    def animate(self, frame):
        ...
        for car in self.ramp:
            car_objects.append(
                plt.Rectangle((car.position, car.y), 5.2, 3, color='red'))  # use a different color for ramp cars
            self.axis.add_patch(car_objects[len(car_objects) - 1])

        return car_objects

    def run(self):
        self.anim = FuncAnimation(self.fig, self.animate, frames=1000, interval=70, blit=True)
        self.fig.canvas.mpl_connect('close_event', lambda a: os._exit(1))

        plt.show()

        plt.figure()
        plt.hist(self.waiting_list, bins=15, color='skyblue', edgecolor='black')
